...
import tflearnface.common
import numpy as np
from tensorflow.examples.tutorials.mnist import input_data
import tensorflow as tf
import cv2
import  tflearnface.tensorflow_face_conv as myconv
...
def generateface(pairdirs):
    ...
    for inputdir, outputdir in pairdirs:
        print("inputdir, outputdir",inputdir, outputdir)
...
def getfileandlabel(filedir):
    ''' get path and host paire and class index to name
        dictdir     example::  { xxj : filedir/xxj  ................}
    '''
...
def main(_):
    ...
    if os.path.exists(savepath+'.meta') is False:
        isneedtrain = True
    if isneedtrain:#没有历史模型
        #first generate all face
        log.debug('generateface')
        generateface([['./image/trainimages', './image/trainfaces']])
        '''
            pathlabelpair(onehot编码):: [('./image/trainfaces\\aaa', [1.0, 0.0, 0.0, 0.0]), ('./image/trainfaces\\wly', [0.0, 1.0, 0.0, 0.0]), ('./image/trainfaces\\xxj', [0.0, 0.0, 1.0, 0.0]), ('./image/trainfaces\\xyq', [0.0, 0.0, 0.0, 1.0])]
            indexoname:: {0: 'aaa', 1: 'wly', 2: 'xxj', 3: 'xyq'}
        '''
        pathlabelpair, indextoname = getfileandlabel('./image/trainfaces')
        #train_x::图片张量      train_y:onehot路径编码
